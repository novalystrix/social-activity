...
export async function GET(req: NextRequest, { params }: RouteContext) {
  ...
  const item = await prisma.corpus.findFirst({ where: { id, accountId } });
...
export async function PUT(req: NextRequest, { params }: RouteContext) {
  ...
  const item = await prisma.corpus.updateMany({
    where: { id, accountId },
    data: { ...(content !== undefined ? { content } : {}), ...(title ? { title } : {}), updatedAt: new Date() },
  });
