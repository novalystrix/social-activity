...
export default function CorpusPage() {
  ...
  useEffect(() => {
    fetch('/api/corpus').then(r => r.json()).then(d => setFiles(d.files || []));
  }, []);

  async function selectFile(id: number) {
    const r = await fetch(`/api/corpus/${id}`);
  ...
  async function saveEdit() {
    if (!selected) return;
    const r = await fetch(`/api/corpus/${selected.id}`, {
      method: 'PUT',
  ...
  const filtered = files.filter(f =>
    !search || f.title.toLowerCase().includes(search.toLowerCase()) || f.filename.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">All files that inform content creation</p>
      </div>
      ...
      <input
        ...
        className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2 text-sm text-zinc-200 focus:border-[#4FC3F7] focus:outline-none"
      />
            ...
            <div key={file.id} className="group relative">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <PinButton item={{ type: 'corpus', id: file.id, snippet: file.title }} />
                </div>
